...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        # size1 = (input_width*num_input_channels-kernel_sizes[0])//strides[0]+1
        # size2 = (size1-kernel_sizes[1])//strides[1]+1
        # size3 = (size2-kernel_sizes[2])//strides[2]+1
        # print(size3)
    ...
    def forward(self, A):
        ...
        for i in range(len(self.convolutional_layers)):
            layer = self.convolutional_layers[i]
            A = layer.forward(A)
    ...
    def backward(self, labels):
        ...
        layer = self.convolutional_layers[2]
        grad = layer.backward(grad*self.activations[2].backward())
        layer = self.convolutional_layers[1]
        grad = layer.backward(grad*self.activations[1].backward())
        layer = self.convolutional_layers[0]
        grad = layer.backward(grad*self.activations[0].backward())
